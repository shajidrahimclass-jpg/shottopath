...
-- Drop old function to force PostgREST schema cache full refresh
DROP FUNCTION IF EXISTS public.process_checkout(jsonb, jsonb, uuid);

-- Recreate with a fresh name to bypass any stale PostgREST cache
...
BEGIN
  ...
  VALUES (
    ...
    CASE WHEN (p_order->>'payment_amount') IS NOT NULL
         THEN (p_order->>'payment_amount')::numeric END,
    p_order->'payment_details',
    p_order->>'transaction_id',
    v_subtotal, v_delivery, v_discount, v_total,
    p_order->>'delivery_address',
    CASE WHEN (p_order->>'delivery_location_id') IS NOT NULL
