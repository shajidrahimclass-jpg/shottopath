...
const MAX_RESOLUTION = 1920; // allow full HD, compress to fit under 1MB
...
export const processImage = async (file: File): Promise<CompressionResult> => {
  ...
  // Always compress large images (>12MP or >1MB) to WebP
  const needsCompression = originalSize > MAX_FILE_SIZE;

  // Also check resolution — load image to inspect dimensions
  const hasMegapixelCheck = await new Promise<boolean>((resolve) => {
    const img = new Image();
    img.onload = () => resolve(img.width * img.height > 8_000_000); // >8MP triggers resize
    img.onerror = () => resolve(false);
    img.src = URL.createObjectURL(file);
  });

  if (!needsCompression && !hasMegapixelCheck) {
    return {
      file,
      wasCompressed: false,
      originalSize,
