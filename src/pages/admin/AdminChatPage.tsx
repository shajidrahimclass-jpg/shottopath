...
export default function AdminChatPage() {
  ...
  const handleSendMessage = async () => {
    ...
    // Handle disappearing chat commands
    if (newMessage.trim() === '/off' || newMessage.trim() === '/on') {
      try {
        setSending(true);
        const isDisappearing = newMessage.trim() === '/on';
        ...
        if (!isDisappearing) {
          // Admin requested to turn off disappearing chat, so we also wipe history as requested
        ...
        if (order) {
          setOrder({ ...order, disappearing_chat: isDisappearing });
        }
        
        setNewMessage('');
        toast.success(isDisappearing ? 'Disappearing chat activated' : 'Disappearing chat deactivated and history cleared');
      } catch (error) {
        console.error('Failed to toggle disappearing chat:', error);
    ...
    <div className="flex h-screen bg-gradient-to-br from-background via-background to-muted/20">
      ...
      <div className="flex-1 flex flex-col">
        ...
        {order?.disappearing_chat && (
          <div className="bg-destructive/10 text-destructive text-xs py-1.5 px-3 text-center border-b border-destructive/20 font-medium flex items-center justify-center gap-2">
