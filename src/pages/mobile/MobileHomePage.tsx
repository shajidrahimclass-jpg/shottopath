          ...
          >
            {banners.map((b, i) => (
              <div key={b.id}
                ...
                <img src={b.image_url ?? ''} alt={b.title ?? undefined} loading={i === 0 ? "eager" : "lazy"} decoding="async" className="w-full h-full object-cover object-center" />
              </div>
            ))}
            ...
            <div className="grid grid-cols-2 gap-3 px-4">
              {products.map((p) => (
                <div key={p.id}
                  ...
                  {/* Full-bleed image */}
                  {getThumb(p) ? (
                    <img src={getThumb(p)} alt={p.name} loading="lazy" decoding="async" className="absolute inset-0 w-full h-full object-cover" />
                  ) : (
              ...
              {products.slice(0, 7).map(p => (
                <div key={`new-${p.id}`}
                  ...
                  <div className="relative aspect-square rounded-2xl overflow-hidden bg-muted mb-1.5 shadow-sm">
                    {getThumb(p) ? (
                      <img src={getThumb(p)} alt={p.name} loading="lazy" decoding="async" className="w-full h-full object-cover" />
                    ) : (
