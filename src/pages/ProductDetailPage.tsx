...
export default function ProductDetailPage() {
  ...
  const displayImages = useMemo(() => {
    if (!product) return [];
    const galleryImages = getDeviceImages(
      deviceType,
      product.pc_images,
      product.mobile_images
    ) || [];
    // Prepend thumbnail as the main embedded image if it exists and isn't already in the gallery
    ...
    // If no gallery at all, fall back to image_url
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // Load related products; reviews load independently so they never block product display
        const allProducts = await getProducts(8);

        setProduct(productData);

        // Add to recently viewed (auth-optional, silent fail)
        addToRecentlyViewed(productData.id).catch(() => {});
        ...
        // Load reviews separately — failure shows empty list, never crashes page
