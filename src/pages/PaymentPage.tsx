...
import { getActiveTerms, checkoutOrder, createNotification } from '@/db/api';
...
interface PaymentData {
  ...
  paymentAmount?: 'full' | 'delivery_only';
  isBuyNow: boolean;
}
...
export default function PaymentPage() {
  ...
  useEffect(() => {
    ...
    if (!user) {
      navigate(isMobileRoute ? MOBILE_ROUTES.login : '/login');
      return;
    }
    ...
    if (!data || !data.orderData) {
      ...
      navigate(isMobileRoute ? MOBILE_ROUTES.checkout : '/checkout');
      return;
    }

    setPaymentData(data);
    // Pre-populate the payment amount choice selected in CheckoutPage
  ...
  const handleCompletePayment = async () => {
    ...
    try {
      ...
      const orderResult = await checkoutOrder(user?.id ?? null, orderDataWithTransaction, paymentData.orderItems);
      ...
      <OrderSuccessModal 
        ...
        onAnimationComplete={() => {
          if (user) {
            ...
            // Get phone from orderData if available
            const phone = paymentData?.orderData?.delivery_address?.phone || '';
