...
export const createDeliveryLocation = async (
  location: Omit<DeliveryLocation, 'id' | 'created_at'>
...
export const deleteDeliveryLocation = async (id: string): Promise<void> => {
  const { error } = await supabase
    .from('delivery_locations')
    .delete()
    .eq('id', id);
...
export const createPaymentGateway = async (
  gateway: Omit<PaymentGateway, 'id' | 'created_at'>
...
export const getProductSalesStats = async (): Promise<Record<string, { totalSold: number, revenue: number }>> => {
  try {
    ...
    if (orderItems) {
      orderItems.forEach(item => {
        if (!stats[item.product_id]) {
          stats[item.product_id] = { totalSold: 0, revenue: 0 };
        }
        stats[item.product_id].totalSold += item.quantity || 1;
        stats[item.product_id].revenue += (item.quantity || 1) * (item.product_price || 0);
      });
    }
    return stats;
  } catch (err) {
...
export const deleteProduct = async (id: string): Promise<void> => {
  ...
  const { data: product } = await supabase
    .from('products')
    .select('image_url, thumbnail_url, additional_images, video_urls')
    .eq('id', id)
    .single();
  ...
  // Cleanup storage if product existed
  if (product) {
    try {
      const getFilePath = (url: string) => {
        ...
        try {
          ...
          // Supabase storage URLs usually end with /storage/v1/object/public/bucketName/filePath
      ...
      if (product.image_url) {
        const path = getFilePath(product.image_url);
        if (path) await supabase.storage.from('products').remove([path]);
      }

      // Delete thumbnail
      if (product.thumbnail_url) {
        const path = getFilePath(product.thumbnail_url);
        if (path) await supabase.storage.from('product-thumbnails').remove([path]);
      }

      // Delete additional images
      if (product.additional_images && product.additional_images.length > 0) {
        const paths = product.additional_images
          .map((url: string) => getFilePath(url))
          .filter(Boolean);
        if (paths.length > 0) {
      ...
      console.error('Error cleaning up product images:', err);
      // We don't throw here to ensure the UI still reflects the successful DB deletion
    }
  }
};
...
export const getProductReviews = async (productIdOrSlug: string): Promise<ReviewWithUser[]> => {
  ...
  // Only do slug lookup when the input is not a UUID
  if (!UUID_REGEX.test(productIdOrSlug)) {
    const { data: product } = await supabase
      .from('products')
      .select('id')
...
// Image Upload API — auto-compresses large/high-res images to WebP ≤1MB
export const uploadImage = async (file: File, folder: string = 'products'): Promise<string> => {
  ...
  if (!prepared.success || !prepared.file) throw new Error(prepared.error || 'Invalid image');

  const fileName = `${folder}/${Date.now()}_${Math.random().toString(36).substring(7)}.webp`;
  const { data, error } = await supabase.storage
    .from('shottopoth_images')
    .upload(fileName, prepared.file);
...
export const uploadBannerImage = async (file: File): Promise<string> => {
  const prepared = await validateAndPrepareImage(file);
  if (!prepared.success || !prepared.file) throw new Error(prepared.error || 'Invalid banner image');
...
export const createCategory = async (category: Omit<Category, 'id' | 'slug' | 'image_url' | 'sort_order' | 'created_at' | 'updated_at'>): Promise<Category> => {
...
export const createRefundsPolicy = async (policy: Omit<RefundsPolicy, 'id' | 'created_at' | 'updated_at'>): Promise<RefundsPolicy> => {
  const { data, error } = await supabase
    .from('refunds_policy')
    .insert(policy)
    .select()
...
const RECENTLY_VIEWED_EXPIRY_DAYS = 14;
